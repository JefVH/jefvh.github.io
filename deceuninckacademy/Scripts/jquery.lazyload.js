...
(function ($, window, document, undefined) {
    $.fn.lazy = function (settings) {
        ...
        var _throttleHandler = _throttle(configuration.throttle, lazyLoadImages);
        ...
        function lazyLoadImages(allImages) {
            ...
            items.filter(function () { return ($(this).closest("body").length > 0); }).each(function () {
                var element = $(this);
            ...
            items = $(items).filter(function () {
                return (!$(this).data("loaded") && $(this).closest("body").length > 0);
            });
        ...
        function _init() {
            ...
            if (configuration.delay < 0 || configuration.combined) {
                ...
                if (configuration.appendScroll !== window) {
                    $(configuration.appendScroll).bind("scroll." + guid, _throttleHandler);
                    $(configuration.appendScroll).bind("resize." + guid, _throttleHandler);
                } else {
                    $(window).bind("scroll." + guid, _throttleHandler);
                    $(window).bind("resize." + guid, _throttleHandler);
                }
            }
        }
        ...
        function _destroyLazy() {
            if (configuration.appendScroll !== window) {
                $(configuration.appendScroll).unbind("scroll." + guid);
                $(configuration.appendScroll).unbind("resize." + guid);
            } else {
                $(window).unbind("scroll." + guid);
