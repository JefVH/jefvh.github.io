...
function acx_slideshow_admin_rename_gallery() 
{
	include('rename_gallery.php');
}
...
function acx_slideshow_admin_actions()
{
	...
	add_menu_page(  'Acurax Slideshow Configuration', __('Slideshow','simple-slideshow-manager'), $acx_slideshow_misc_user_level,'Acurax-Slideshow-Settings' ,'acx_slideshow_admin',plugin_dir_url( __FILE__ ).'/images/acurax_international.png' ); // 8 for admin
	...
	add_submenu_page('', 'Rename Gallery', 'Rename Gallery', $acx_slideshow_misc_user_level, 'Acurax-Slideshow-Rename-Gallery' ,'acx_slideshow_admin_rename_gallery');
	add_submenu_page('Acurax-Slideshow-Settings', __('Generate Shortcode','simple-slideshow-manager'), __('Generate Shortcode','simple-slideshow-manager'), $acx_slideshow_misc_user_level, 'Acurax-Slideshow-Generate-Shortcode' ,'acx_slideshow_generate_shortcode');
...
function acx_slideshow_update( $plugin_data, $r )
{
...
if ( version_compare('2.8', $wp_version, '<=') ) 
{
	...
	if ( 'plugins.php' === $pagenow )
	{
		...
		$folder = basename( dirname( __FILE__ ) );
		$acx_add_slideshow_u_notice = "in_plugin_update_message-{$folder}/{$file}";
		add_action( $acx_add_slideshow_u_notice, 'acx_slideshow_update', 20, 2 );
	}
}
